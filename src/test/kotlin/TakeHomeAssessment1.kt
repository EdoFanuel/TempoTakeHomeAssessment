...
fun Hierarchy.filter(nodeIdPredicate: (Int) -> Boolean): Hierarchy {
    ...
    val validNodes = mutableListOf<Int>()
    val validDepths = mutableListOf<Int>()
    for (i in 0 until size) {
        val nodeVal = nodeId(i)
        val depth = depth(i)
        // Remove non ancestor from the stack
        while (stack.isNotEmpty() && stack.peek().first >= depth) {
            stack.pop()
        }
        // Add current node to stack
        stack.push(depth to nodeVal)
        // Check if everything in stack (ancestor + current node) all pass the predicate
        var match = true
        for (node in stack) {
            if (!nodeIdPredicate(node.second)) {
                match = false
                break
            }
        }
    ...
    return ArrayBasedHierarchy(validNodes.toIntArray(), validDepths.toIntArray())
}
